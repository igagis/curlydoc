...
interpreter::exception::exception(const std::string& message, const std::string& file, const tml::leaf_ext& leaf) :
	std::invalid_argument([&]() {
		const auto& l = leaf.info.location;
		std::stringstream ss;
		ss << message << '\n';
		ss << "    " << file << ":" << l.line << ":" << l.offset << ": " << leaf.string;
		return ss.str();
	}())
{}
...
void interpreter::context::add(const std::string& name, tml::forest_ext&& value)
{
...
const tml::forest_ext& interpreter::context::find(const std::string& name) const
{
...
interpreter::interpreter(std::unique_ptr<fsif::file> file) :
		...
		try {
			...
			if (v.value) {
				...
				if (func_i == this->functions.end()) {
					throw exception(std::string("function/macro '") + i->value.string + "' not found");
				}
				...
				if (!output.empty()) {
					output.front().value.info.flags.set(tml::flag::space, i->value.info.flags.get(tml::flag::space));
				}
			}
