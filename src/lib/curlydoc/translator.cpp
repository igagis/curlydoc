...
translator::translator() {
  ...
  this->add_tag(
      "p", [this](bool space, auto &forest) { this->on_paragraph(forest); });
  ...
  this->add_tag("h1",
                [this](bool space, auto &forest) { this->on_header1(forest); });

  this->add_tag("h2",
                [this](bool space, auto &forest) { this->on_header2(forest); });

  this->add_tag("h3",
                [this](bool space, auto &forest) { this->on_header3(forest); });

  this->add_tag("h4",
                [this](bool space, auto &forest) { this->on_header4(forest); });

  this->add_tag("h5",
                [this](bool space, auto &forest) { this->on_header5(forest); });

  this->add_tag("h6",
                [this](bool space, auto &forest) { this->on_header6(forest); });

  this->add_tag("ins",
                [this](bool space, auto &forest) { this->on_ins(forest); });
...
const std::string &translator::get_parent_tag() const noexcept {
  ...
  for (auto i = std::next(this->cur_tag.rbegin()); i != this->cur_tag.rend();
       ++i) {
    if (!i->empty()) {
      // std::cout << "ret = " << *i << '\n';
      return *i;
...
void translator::translate(bool space, const tml::tree_ext &tree) {
  const auto &tag = tree.value.string;
  ...
  utki::scope_exit cur_tag_scope_exit([this]() { this->cur_tag.pop_back(); });
  ...
  try {
    ...
    std::stringstream ss;
    ss << e.what() << " at:" << '\n';
    ss << "    " << tree.value.info.location.line << ":"
       << tree.value.info.location.offset << ": "
       << (tag.empty() ? "\"\"" : tag);
    throw std::invalid_argument(ss.str());
  }
}

void translator::translate(tml::forest_ext::const_iterator begin,
                           tml::forest_ext::const_iterator end) {
  for (auto i = begin; i != end; ++i) {
    bool space = i != begin && i->value.info.flags.get(tml::flag::space);

    if (i->children.empty()) {
      this->report_space(space);
      this->on_word(i->value.string);
      continue;
    }
...
bool translator::is_parameters(const tml::tree_ext &tree) noexcept {
  return tree == "prm" && !tree.children.empty();
}

void translator::check_parameter(const tml::tree_ext &tree) {
  if (tree.children.empty()) {
    throw std::invalid_argument(std::string("no value specified for '") +
                                tree.value.string + "' parameter");
  }
}

void translator::handle_image(const tml::forest_ext &forest) {
  image_params params;
...
translator::valign parse_valign(const std::string &str) {
  ...
  throw std::invalid_argument(
      std::string("unknown vertical alignment keyword: ") + str);
}
} // namespace

void translator::handle_table(const tml::forest_ext &forest) {
  ASSERT(!forest.empty())
  auto i = forest.begin();

  this->cur_table.emplace_back();
  utki::scope_exit cur_table_scope_exit(
      [this]() { this->cur_table.pop_back(); });
    ...
    for (const auto &p : i->children) {
      ...
      if (p == "cols") {
        ...
        for (const auto &a : p.children) {
          t.aligns.push_back(parse_align(a.value.string));
        }
      } else if (p == "valign") {
        for (const auto &a : p.children) {
          t.valigns.push_back(parse_valign(a.value.string));
        }
      }
    }
    ++i;
...
bool translator::table_row::is_free_span(
    decltype(occupied_cols)::const_iterator iter, size_t span) const noexcept {
  size_t n = 0;
...
size_t translator::table_row::get_free_col_index(size_t span) {
  for (auto i = this->occupied_cols.begin(); i != this->occupied_cols.end();
       ++i) {
    if (!(*i)) {
...
void translator::table_row::set_occupied(size_t index, size_t span) {
  ...
  for (auto i = utki::next(this->occupied_cols.begin(), index);
       i != utki::next(this->occupied_cols.begin(), index + span); ++i) {
    ASSERT(!(*i))
    *i = true;
  }
  this->span += span;
...
void translator::table::push(cell &&c) {
  ...
  if (row.span > this->num_cols) {
    std::stringstream ss;
    ss << "table number of columns (" << this->num_cols
       << ") and cells row span (" << row.span << ") mismatch for row #"
       << (this->cur_row + 1);
    throw std::invalid_argument(ss.str());
  }
...
void translator::handle_cell(const tml::forest_ext &forest) {
  ASSERT(this->cur_tag.size() >= 2)

  if (this->get_parent_tag() != table_tag) {
    throw std::invalid_argument(
        "'cell' tag is only allowed directly inside of 'table' tag");
  }
...
void translator::handle_list(const tml::forest_ext &forest) {
  this->cur_list.emplace_back();
  utki::scope_exit cur_list_scope_exit([this]() { this->cur_list.pop_back(); });
...
void translator::handle_list_item(const tml::forest_ext &forest) {
  ASSERT(this->cur_tag.size() >= 2)

  if (this->get_parent_tag() != list_tag) {
    throw std::invalid_argument(
        "'li' tag is only allowed directly inside of 'list' tag");
  }

  ASSERT(
      !this->cur_list.empty()) // since we have checked the parent tag is 'list'
